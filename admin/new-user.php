...
<body>
          ...
          <div class="container">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 space-y-6">
              <div class="flex justify-between items-center">
                <div>
                  <p class="text-lg font-semibold text-blue-500">Notifications</p>
                </div>
                <button class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Clear</button>
              </div>
  ...
  <script>
    function fetchNotification() {
      $.ajax({
        url: 'notification-config.php',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
          if (response.status === 'success') {
            var NotificationItems = response.data;
            var NotificationHTML = '';

            NotificationItems.forEach(function(item) {
              NotificationHTML += `
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pb-4">
                            <img src="../image/users/${item.img}" alt="Product Image" class="w-1/4 h-auto md:w-20">
                            <div class="col-span-2">
                                <h3 class="font-bold text-lg text-red-600 dark:text-red-600">Great news! A new user just joined your webpage!</h3>
                                <h3 class="font-bold text-lg text-gray-700 dark:text-gray-200">${item.username}</h3>
                                <p class="text-sm text-gray-500 dark:text-gray-400">${item.email}</p>
                                <p class="text-lg font-semibold text-blue-600">${item.name}</p>
                            </div>
                            <div class="md:text-right">
                             <form>
                               <button type="button" class="bg-gray-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg">Delete</button>
                             </form>
                            </div>
                        </div>`;
            });
            ...
            $('#notification-container').html('<h1>No Notifications</h1>');
          }
        },
        error: function() {
